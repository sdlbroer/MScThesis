...
source('Z:/Documents/Scripts/data_management_v12.R')
...
time.model <- 'lmm' # lmm/mm1/mm2
assoc.param <- 'slope' # slope/tv
if(time.model == 'lmm' & assoc.param == 'slope') {seed <- floor(1803158/110) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'lmm' & assoc.param == 'tv') {seed <- floor(1803158/111) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'slope') {seed <- floor(1803158/112) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'tv') {seed <- floor(1803158/113) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm2' & assoc.param == 'slope') {seed <- floor(1803158/114) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm2' & assoc.param == 'tv') {seed <- floor(1803158/115) ; seeds <- seed:(seed + n.RCV)} # set seeds
...
if(time.model == 'mm1'){ # splines with 1 internal knot
  fixed.formula.PSA <- as.formula('log2PSA ~ ns(fup_time, k = c(2), B = c(0, 18.5))*therapy_received')
  fixed.formula.LDH <- as.formula('log2LDH ~ ns(fup_time, k = c(2), B = c(0, 18.5))*therapy_received')
  fixed.formula.ALP <- as.formula('log2ALP ~ ns(fup_time, k = c(2), B = c(0, 18.5))*therapy_received')
  random.formula <- as.formula('~ ns(fup_time, k = c(2), B = c(0, 18.5)) | id_num')
}
if(time.model == 'mm2'){ # splines with 2 internal knots
  fixed.formula.PSA <- as.formula('log2PSA ~ ns(fup_time, k = c(2,4), B = c(0, 18.5))*therapy_received')
  fixed.formula.LDH <- as.formula('log2LDH ~ ns(fup_time, k = c(2,4), B = c(0, 18.5))*therapy_received')
  fixed.formula.ALP <- as.formula('log2ALP ~ ns(fup_time, k = c(2,4), B = c(0, 18.5))*therapy_received')
  random.formula <- as.formula('~ ns(fup_time, k = c(2,4), B = c(0, 18.5)) | id_num')
}
...
if(assoc.param == 'tv') def.form <- ~ value(log2PSA) * ns(fup_time, k = c(2, 4), B = c(0, 18.5)) +
  value(log2LDH) * ns(fup_time, k = c(2, 4), B = c(0, 18.5)) +
  value(log2ALP) * ns(fup_time, k = c(2, 4), B = c(0, 18.5)) # time-varying 
  ...
  for(i in 1:n.folds){
    ...
    if(!inherits(train_longit_LDH, "try-error") & !inherits(train_longit_ALP, "try-error")){
      ...
      train_joint <- jm(train_surv, list(train_longit_PSA, train_longit_LDH, train_longit_ALP), 
                        time_var = "fup_time", functional_forms = def.form)
...
## export dataframe
write.csv(pp, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/multlong_pp_SPJM_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
write.csv(all_cindex, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/multlong_cindex_SPJM_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
