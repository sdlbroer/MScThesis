...
seed <- floor(1803158/101) # set seed 
...
surv_strict_landmarked <- left_join(surv_strict_landmarked,
                                    select(long_strict_landmarked, id_num, log2PSA, log2ALP, log2LDH),
                                    by = 'id_num')
...
for(m in 1:n.RCV){
  ...
  for(i in 1:n.folds){
    ...
    model_landmark_LOCF <- coxph(Surv(time_obs, NLCB_overall_num) ~ therapy_received + log2PSA + log2ALP + log2LDH, 
                                 data = df_train_surv, x = TRUE)
    
    # validate model on test set 
    ## predict survival outcome
    fail_prob <- as.data.frame(1 - predictSurvProb(model_landmark_LOCF, newdata = df_test_surv, times = times))
...
## export dataframe
write.csv(pp, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/mult_longvariable/pp_strictlandmarking_LOCF_20x5.csv', row.names = FALSE)
write.csv(all_cindex, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/mult_longvariable/cindex_strictlandmarking_LOCF_20x5.csv', row.names = FALSE)
