...
# load functions
source('Z:/Documents/Scripts/RQ3_prediction/dynamic_prediction/final_onevariable/create_folds_prop_function.R')
...
if(time.model == 'lmm' & assoc.param == 'slope') {seed <- floor(1803158/10) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'lmm' & assoc.param == 'tv') {seed <- floor(1803158/13) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'slope') {seed <- floor(1803158/11) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'tv') {seed <- floor(1803158/14) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm2' & assoc.param == 'slope') {seed <- floor(1803158/22) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm2' & assoc.param == 'tv') {seed <- floor(1803158/15) ; seeds <- seed:(seed + n.RCV)} # set seeds
...
if(time.model == 'lmm'){ # linear
  fixed.formula <- as.formula('log2PSA ~ fup_time*therapy_received')
  random.formula <- as.formula('~ fup_time | id_num')
}
if(time.model == 'mm1'){ # splines with 1 internal knot
  fixed.formula <- as.formula('log2PSA ~ ns(fup_time, k = c(2), B = c(0, 18.5))*therapy_received')
  random.formula <- as.formula('~ ns(fup_time, k = c(2), B = c(0, 18.5)) | id_num')
}
if(time.model == 'mm2'){ # splines with 2 internal knots
  fixed.formula <- as.formula('log2PSA ~ ns(fup_time, k = c(2,4), B = c(0, 18.5))*therapy_received')
...
for(m in 1:n.RCV){
  ...
  for(i in 1:n.folds){
    ...
    train_longit <- lme(fixed = fixed.formula, 
                        random = random.formula,
                        data = df_train_long,
                        control = lmeControl(opt = 'optim'))
    ## joint model, time-varying
    train_joint <- jm(train_surv, train_longit, time_var = "fup_time",
                      functional_forms = def.form)
...
## export dataframe
write.csv(pp, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/pp_SPJM_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
write.csv(all_cindex, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/cindex_SPJM_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
