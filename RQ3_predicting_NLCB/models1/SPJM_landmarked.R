...
if(time.model == 'lmm' & assoc.param == 'slope') {seed <- floor(1803158/16) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'lmm' & assoc.param == 'tv') {seed <- floor(1803158/17) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'slope') {seed <- floor(1803158/18) ; seeds <- seed:(seed + n.RCV)} # set seeds
if(time.model == 'mm1' & assoc.param == 'tv') {seed <- floor(1803158/19) ; seeds <- seed:(seed + n.RCV)} # set seeds
...
longdata <- psa_long_train %>%
  ...
  filter(time_obs > t0) %>% # landmark data (strict)
  filter(fup_time <= t0)
...
if(time.model == 'mm1'){ # splines with 1 internal knot
  fixed.formula <- as.formula('log2PSA ~ ns(fup_time, k = c(2), B = c(0, 3.8))*therapy_received')
...
if(assoc.param == 'tv') def.form <- ~ value(log2PSA) * ns(fup_time, k = c(2), B = c(0, 3.8)) # time-varying 
...
## export dataframe
write.csv(pp, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/pp_SPJM_lmark_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
write.csv(all_cindex, paste0('Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/cindex_SPJM_lmark_', time.model,'_', assoc.param, '_20x5.csv'), row.names = FALSE)
