...
seed <- floor(1803158/204) # set seed 
...
survdata <- survdata[complete.cases(survdata) & survdata$time > t0,]
longdata <- longdata[longdata$fup_time <= t0 & longdata$id %in% survdata$id,]
...
for(m in 1:n.RCV){
  ...
  for(i in 1:n.folds){
    ...
    model_pcr <- fit_prclmm(object = sum_lme, surv.data = df_train_surv,
                            baseline.covs = ~ therapy_received + Gleason + LocationMetastases + ecog + treatmentline,
...
## export dataframe
write.csv(pp, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/mult_baseline/pp_pencal_ridge_20x5.csv', row.names = FALSE)
write.csv(all_cindex, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/mult_baseline/cindex_pencal_ridge_20x5.csv', row.names = FALSE)
