...
longdata <- psa_long_train %>%
  ...
  select(id_num, therapy_received, fup_time = time, log2PSA, time_obs)
...
for(m in 1:n.RCV){
  ...
  for(i in 1:n.folds){
    ...
    ## fit Cox model 
    model_cox <- coxph(Surv(time_obs, NLCB_overall_num) ~ therapy_received + predPSA, 
    ...
    fail_prob <- as.data.frame(1 - predictSurvProb(model_cox, newdata = df_test_surv[,c('therapy_received', 'predPSA')], times = times))
...
## export dataframe
write.csv(pp, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/pp_strictlandmarking_LMM_20x5.csv', row.names = FALSE)
write.csv(all_cindex, 'Z:/Documents/Scripts/RQ3_prediction/saved_dataframes/cindex_strictlandmarking_LMM_20x5.csv', row.names = FALSE)
